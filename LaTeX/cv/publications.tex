% CV only. The resume shows a short selection instead (cv/selected.tex).
%
% The counts and metrics come from cv/facts.tex and are verified against these
% \nocite lists by the build.
%
% The \newpage that used to sit before Patents was pushing them onto a page of
% their own with two thirds of it blank; the section headings carry the
% structure well enough without it.

\cvsection{Publications}

\begin{cvparagraph}
    \cvBooks{} edited book \enskip\textbullet\enskip \cvChapters{} book chapters \enskip\textbullet\enskip \cvJournals{} journal articles \enskip\textbullet\enskip \cvConfPapers{} conference papers \enskip\textbullet\enskip \cvPatents{} granted patents.

    \vspace{0.8mm}
    \cvCitations{} citations \enskip\textbullet\enskip h-index \cvHIndex{} \enskip\textbullet\enskip i10-index \cvITenIndex{} \enskip\textbullet\enskip \cvurl{\cvScholarURL}{Google Scholar}, \cvScholarAsOf.
\end{cvparagraph}

\cvsubsection{Book}

\begin{refsection}
	\nocite{Radar2022}
	{\color{text}\printbibliography[heading=none, type=book]}
\end{refsection}

\cvsubsection{Book Chapters}

\begin{refsection}
	\nocite{Peng2020, Peng2019, Gomez-Garcia2018}
	{\color{text}\printbibliography[heading=none, type=book]}
\end{refsection}

\cvsubsection{Journal Articles}
\begin{refsection}
	\nocite{TIM2018, Sensors2019, SJ2018, TMTT2018SAR, TMTT2018MIMO, TMTT2017Array, JERM2017review, TMTT2017review, TMTT2017FMCW, TIM2016, Sensors2016, TMTT2016, TMTT2016Beamforming, MM2015, TMTT2014, AWPL2013}
	{\color{text}\printbibliography[heading=none, type=article]}
\end{refsection}

\cvsubsection{Conference Proceedings}
\begin{refsection}
	\nocite{NDSS2023, TELSIKS2019, IMBioC2019, WiSNet2019, DCAS2018, Sensys2018, ACES2018, APS2018, RASC2018, I2MTC20182, I2MTC20181, IWS2018, EuCAP2018, NRSM2018, APMC2017, ACES2017, ACES20172, ACES20171, IMBIOC2017, WiSNet20171, WiSNet20172, NRSM2017, RFIT2016, IMS20161, IMS2016, IWS2016, IWS20161, BioWireleSS2016, BioWireleSS20161, RWS2016, IMWS2015, MWSCAS2015}
	{\color{text}\printbibliography[heading=none, type=inproceedings]}
\end{refsection}

\cvsubsection{Patents}
\begin{refsection}
	\nocite{SLOWTIME2024, ARRAY2023, FREQUENCY2022, OBJECT2021, COMPLEX2021, DIGITAL2019, ANALOG2019, OMNI2015, ARRAY2015, TRAY2013}
	{\color{text}\printbibliography[heading=none, type=patent]}
\end{refsection}
